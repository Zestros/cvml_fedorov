...
class SimpleDetector(nn.Module):
    ...
    def __init__(self, num_classes=3):
        ...
        """self.fc = nn.Sequential(
            nn.Flatten(),
            nn.Linear(1024, 256), # можно разделить ветку на две разые для скорости обучения 
            nn.ReLU(),
            nn.Dropout(0.3),
        )"""
    ...
    def forward(self, x):
        ...
        bbox_coords = self.bbox(features)
        return cls_logits, bbox_coords


"""def giou_loss(pred, target): # Можно доделать чтобы ширина и высота менялись то есть добавить размер 
...
def ciou_loss(pred, target):
    ...
    dist_sq = (pred[:, 0] - target[:, 0])**2 + (pred[:, 1] - target[:, 1])**2
...
def detection_loss(cls_pred, bbox_pred, cls_targets, bbox_targets, lambda_bbox=30.0):
    ...
    loss_bbox += ciou_loss(bbox_pred, bbox_targets)
...
optimizer = torch.optim.Adam([
    ...
    {'params': model.bbox.parameters(), 'lr': 1e-3}
])
...
if save_path.exists():
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loss_avg <= loss_threshold or no_improve >= no_improve_count:
            break
        """ 
        if val_acc > best_acc:
            best_acc = val_acc
